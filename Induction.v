...
Proof.
  ...
  Case "n = S n'". simpl. rewrite mult_n_Sm. rewrite IHn'. reflexivity.
Qed.
...
Proof.
  ...
  Case "n = S n'".
    ...
    SCase "Proof of assertion". simpl. reflexivity. rewrite H.
...
Proof.
  intros n. induction n as [| n'].
  Case "n = 0". simpl. reflexivity.
  Case "n = S n'". simpl. rewrite plus_0_r. reflexivity.
Qed.
...
Proof.
  ...
  Case "p = 0". simpl. rewrite mult_0_r. rewrite mult_0_r. rewrite mult_0_r. reflexivity.
  Case "p = S p'".
    assert (H1: forall k, k * S p' = S p' * k).
    SCase "assertion". intros k. rewrite -> mult_comm. reflexivity.
    ...
    SCase "assertion". rewrite -> plus_comm. rewrite -> plus_assoc. reflexivity.
    rewrite -> H3.
    assert (H4: n + m = m + n).
    SCase "assertion". rewrite -> plus_comm. reflexivity.
...
Proof.
  intros n m p.
  rewrite plus_assoc. rewrite plus_assoc.
  replace (n + m) with (m + n). SCase "assertion". rewrite plus_comm. reflexivity.
  rewrite plus_comm. reflexivity.
Qed.
(** [] *)
