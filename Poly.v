...
This is a confusing question. I think the answer is 0, since it's impossible
to actually construct a value of type [baz].
...
Proof.
  intros x v s. induction s as [|y ys].
  Case "s = []". simpl. reflexivity.
  Case "s = y::ys". simpl. rewrite IHys. simpl. reflexivity.
Qed.
...
Proof.
  intros X l. induction l as [| x xs].
  Case "l = []". simpl. reflexivity.
  Case "l = x::xs". simpl. rewrite rev_snoc. rewrite IHxs. reflexivity.
Qed.
...
Proof.
  intros X Y Z f x y. unfold prod_uncurry. unfold prod_curry. reflexivity.
Qed.
...
Proof.
  ...
  assert (H: (fst p, snd p) = p). destruct p. reflexivity. rewrite H. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros X Y. intros f l. induction l as [| x xs].
  Case "l = []". simpl. reflexivity.
  Case "l = x::xs". simpl. rewrite map_snoc. rewrite IHxs. reflexivity.
Qed.
...
Proof.
  intros b. induction b. reflexivity. reflexivity.
Qed.
...
Proof.
  intros X. intros x1 c2 k1 k2. intros f. intros.
...
Proof.
  ...
  Case "l = []". simpl. reflexivity.
  Case "l = x::xs". simpl. rewrite <- IHxs. unfold fold_length. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. unfold fold_map. induction l. simpl. reflexivity.
