...
Theorem bag_theorem :
  forall b : bag, forall n : nat, count n (add n b) = S (count n b).
Proof.
  intros b n. simpl. rewrite <- beq_nat_refl. reflexivity.
Qed.
...
Proof.
  ...
  Case "l = nil". reflexivity.
  Case "l = h::t". simpl.
    assert (H: forall n : nat, forall l : natlist, rev (snoc l n) = n :: rev l).
    SCase "assertion". intros n l. induction l as [| h' t'].
      SSCase "l = nil". reflexivity.
      SSCase "l = h'::t'". simpl. rewrite IHt'. reflexivity.
...
Proof.
  intros l1 l2. induction l1 as [| h t].
    ...
    Case "l1 = h::t". simpl.  rewrite snoc_append. rewrite IHt. rewrite snoc_append.
...
Proof.
  ...
  Case "l1 = h::t". simpl. rewrite IHt. destruct h.
    SCase "h = 0". reflexivity.
    SCase "h = S h'". simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  Case "l = nil". reflexivity.
  Case "l = x::xs". simpl. rewrite <- IHxs.
    assert (H: beq_nat x x = true). induction x as [| x'].
    SCase "x = 0". reflexivity.
    SCase "x = S x'". simpl. rewrite IHx'. reflexivity.
...
Theorem cons_snoc_app :
  forall l1 l2 : natlist, forall n : nat,
    snoc (l1 ++ l2) n = rev (n :: ((rev l2) ++ (rev l1))).
Proof.
  intros l1 l2 n. rewrite snoc_append. rewrite app_assoc.
  rewrite <- distr_rev.
  assert (H: forall x y, rev (x :: rev y) = y ++ [x]). intros x y.
  induction y. reflexivity. simpl. rewrite snoc_append. rewrite snoc_append. rewrite distr_rev.
...
Proof.
  ...
  Case "s = x::xs". simpl.
  induction x as [| x'].
    SCase "x = 0". simpl. rewrite ble_n_Sn. reflexivity.
...
Theorem bag_count_sum :
  forall b1 b2 : bag, forall n : nat,
    count n (add n (rev (sum b1 b2))) = S (count n (sum (rev b2) (rev b1))).
Proof.
  intros b1 b2 n. simpl.
  rewrite <- beq_nat_refl. rewrite distr_rev. unfold sum. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros l default. induction l. reflexivity. reflexivity.
Qed.
(** [] *)
